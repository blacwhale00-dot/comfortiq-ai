...
const TIER_MESSAGES = {
  hot: {
    opening: (name?: string) =>
      `${name ? `Hey ${name},` : "Hey there,"} I'm going to be straight with you. Based on everything in your assessment, your HVAC system needs attention soon. Like, within the next few months. The good news? We caught it before it became an emergency.`,
    urgency:
    ...
    nextStep:
      "I want to get you a free in-home assessment so we can see exactly what we're dealing with. No obligation, just William coming out to take a look and give you a straight answer on whether to repair or replace. Want to book that?",
  },
  warm: {
    opening: (name?: string) =>
...
export function analyzeHVACPhoto(imageDescription: string): string {
  ...
  const hasDents = lower.includes("dent");
  const hasYear = lower.match(/\b(19|20)\d{2}\b/);
  const hasCarrier = lower.includes("carrier");
  const hasTrane = lower.includes("trane");
  const hasLennox = lower.includes("lennox");
  const hasCompressorDamage = lower.includes("compressor") && lower.includes("damage");
  const hasCorrosion = lower.includes("corrosion") || lower.includes("corroded");
  ...
  if (hasCorrosion || (hasRust && lower.includes("coil"))) {
  ...
  if (hasYear) {
    const yearStr = hasYear[0];
    const year = parseInt(yearStr);
    const age = 2026 - year;
    if (age >= 18) {
      return `The serial number tells me this unit was installed in ${year}. That's ${age} years old, well past the typical 12-15 year lifespan. At this point, you're in nursing it along territory. This is when replacement starts becoming cheaper than continued repairs.`;
    } else if (age >= 12) {
      return `Unit from ${year}, about ${age} years old. Right in that window where systems start showing their age. Nothing critical yet, but you're entering the zone where I'd start planning for a replacement within the next 1-2 years.`;
    }
  }
  if (hasCarrier || hasTrane || hasLennox) {
    return "That's a solid brand. The age matters more than the brand at this point. What does the serial number sticker say? That'll tell us exactly how old this unit is and what we're working with.";
  }
  if (hasDents || hasRust) {
...
export function analyzeBreakerPanel(imageDescription: string): string {
  const lower = imageDescription.toLowerCase();
  const isFederalPacific = lower.includes("federal pacific") || lower.includes("fpe");
  const isZinsco = lower.includes("zinsco");
  const isSquareD = lower.includes("square d");
  const isEaton = lower.includes("eaton") || lower.includes("ch");
  const isMurray = lower.includes("murray");
  const hasDoubleTaps = lower.includes("double tap") || lower.includes("double-tap");
  const hasRust = lower.includes("rust") || lower.includes("corrosion");
  const hasBurnMarks = lower.includes("burn mark") || lower.includes("burn mark");
...
export function analyzeWaterHeater(imageDescription: string): string {
  const lower = imageDescription.toLowerCase();
  const hasTank = lower.includes("tank") && !lower.includes("tankless");
  const hasTankless = lower.includes("tankless") || lower.includes("tank-less");
  const hasRust = lower.includes("rust");
  const hasLeakSigns = lower.includes("leak") || lower.includes("leak") || lower.includes("water damage");
  const hasSediment = lower.includes("sediment") || lower.includes("debris");
  const hasYear = lower.match(/\b(19|20)\d{2}\b/);
  ...
  if (hasTankless) {
    return "That's a tankless water heater. Those are great. They typically last 20+ years if maintained properly. The age question is different for tankless. Do you know roughly when it was installed?";
  }
...
export function generateComfortResponse(
  ...
  if (
    ...
    if (lower.includes("breaker") || lower.includes("panel") || lower.includes("electrical")) {
      return "Go ahead and upload a photo of your breaker panel. I'm looking for the brand name and any warning signs like rust, double-tapped breakers, or burn marks. Upload what you've got.";
    }
    if (
  ...
  if (
    ...
    return "Let me put together a no-obligation quote for you. It takes about two minutes. I'll need to know your home size and what tier you're interested in. I'll show you exactly what it costs, what your monthly payment would be, and what kind of energy savings you'd get. Fair?";
  }
  ...
  if (
    ...
    if (
      context.systemAge === ">15" ||
      (context.systemAge === "12-15" &&
        (context.billStatus === "high" || (context.emergencyCount ?? 0) >= 1))
    ) {
  ...
  if (
    ...
    return "The easiest way to tell is to look at the serial number sticker on your outdoor unit. It usually has a date code. Can you take a quick look and tell me what you see?";
  }
