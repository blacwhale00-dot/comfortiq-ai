      ...
      <div className="space-y-3">
        {question.options.map((opt) => (
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 sm:px-5 py-4 rounded-xl border-2 text-left transition-all duration-200 tap-target ${String(value) === opt.value
          ...
          >
            ...
            <span className="text-sm font-medium text-foreground min-w-0">{opt.label}</span>
          </button>
        ))}
      </div>
    </motion.div>
